...
if [ "${patchlines}" == "1" ]; then
    ...
    rosrun unitree_camera point_cloud_node _run_config_file:=./src/unitree_camera/config/stereo_camera_config.yaml camera${camNum} ${timeoffset} &
    sleep 5
    ...
    if [ "${camNum1}" == "2" ]; then
        ...
        rosrun unitree_camera point_cloud_node _run_config_file:=./src/unitree_camera/config/stereo_camera_config.yaml camera${camNum1} ${timeoffset}  &
    fi
    sleep 5
    if [ "${camNum2}" == "2" ]; then
        rosrun unitree_camera example_point ./src/unitree_camera/config/stereo_camera_config1.yaml &
    else
        rosrun unitree_camera point_cloud_node _run_config_file:=./src/unitree_camera/config/stereo_camera_config1.yaml camera${camNum2} ${timeoffset}  &
    fi
    sleep 5
fi
